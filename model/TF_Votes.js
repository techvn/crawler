...
var VotesModel = function() {
    ...
    self.getList = function(fields, con, order, limit, callback) {
        ...
        var sql = "SELECT " + (fields ? fields : '*') + " FROM `" + VoteObject().table
            + "` WHERE " + (con ? con : '1=1')
    ...
    self.getDetail = function(field, con, callback, refer) {
        var conn = utils.getMySql(config);
        var sql = "SELECT " + (field ? field : '*') + " FROM `" + VoteObject().table
            + "` WHERE " + (con ? con : '1=1');
        conn.query(sql, function(err, rows, fields) {
            if(err) { err['sql'] = sql; }
    ...
    self.insertSingle = function(data, callback, refer) {
        ...
        var sql = "INSERT IGNORE INTO " + VoteObject().table
            + "(" + field + ") VALUE(" + value + ")";
        conn.query(sql, function(err, rows, fields) {
            if(err) { err['sql'] = sql; }
            // rows data
...
exports.VotesModel = new VotesModel();